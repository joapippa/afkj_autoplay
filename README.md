# AFK Journey 幻霊挑戦 自動操作ツール

## ファイル構成

```
afkj_autoplay/
├── afk_journey_auto.py   ← メインスクリプト（自動操作）
├── make_templates.py     ← テンプレート画像切り出しツール
├── README.md
├── screenshots/          ← スクショ6枚を置くフォルダ
│   ├── ステージ選択.png
│   ├── 幻霊先鋒ステージ.png
│   ├── クリア編成.png
│   ├── オート挑戦中.png
│   ├── オート戦闘終了.png
│   └── 戦闘敗北.png
└── templates/            ← 切り出したボタン画像（自動作成）
```

---

## 動作フロー

```mermaid
flowchart TD
    A([スクリプト開始]) --> B{Step1: --step1 オプション}
    B -->|幻霊挑戦| B1[幻霊挑戦ボタンをクリック]
    B -->|挑戦| B2[挑戦ボタンをクリック]
    B -->|random| B3[ランダムに選択してクリック]
    B1 --> C[Step2: クリア編成をクリック]
    B2 --> C
    B3 --> C
    C --> D[Step3: 一括適用をクリック]
    D --> E[Step4: オート挑戦をクリック]
    E --> F[Step5: 戦闘終了を待機]
    F --> G{画面を判定}
    G -->|オート戦闘終了を検出| H[Step6-2a: タップで閉じる]
    G -->|戦闘敗北を直接検出| I[Step6-2: もう一度をクリック]
    H --> J[戦闘敗北画面を待機]
    J --> I
    I --> C
```

---

## セットアップ手順

### 1. Python のインストール
https://www.python.org/downloads/ からインストール（3.8以上推奨）

### 2. 必要ライブラリのインストール
コマンドプロンプト（cmd）で実行：
```
pip install pyautogui opencv-python numpy pillow pygetwindow pywin32
```

### 3. スクショの準備
`screenshots/` フォルダを作成し、以下6枚のスクショを入れてください。
ゲームを起動する必要はありません。事前に撮影済みのものでOKです。

| ファイル名 | 画面の状態 | サンプル |
|---|---|---|
| ステージ選択.png | 幻霊挑戦ボタンが見える画面 | ![](screenshots_sample/ステージ選択.png) |
| 幻霊先鋒ステージ.png | クリア編成・オート挑戦ボタンが見える画面 | ![](screenshots_sample/幻霊先鋒ステージ.png) |
| クリア編成.png | 一括適用ボタンが見える画面 | ![](screenshots_sample/クリア編成.png) |
| オート挑戦中.png | 戦闘中の画面 | ![](screenshots_sample/オート挑戦中.png) |
| オート戦闘終了.png | オート戦闘終了の結果画面 | ![](screenshots_sample/オート戦闘終了.png) |
| 戦闘敗北.png | 戦闘敗北の画面 | ![](screenshots_sample/戦闘敗北.png) |

### 4. テンプレート画像の作成（初回のみ）
```
python make_templates.py
```
スクショが順番にウィンドウで開くので、各ボタンをドラッグで選択 → Enter で保存します。
**ゲームの起動・切り替えは不要です。**

#### 切り出すボタン一覧

| テンプレート名 | 使用スクショ | 切り出す場所 |
|---|---|---|
| 幻霊挑戦_btn | ステージ選択.png | 左下の「幻霊挑戦」ボタン |
| 挑戦_btn | ステージ選択.png | 右下の「挑戦」ボタン（数値を除いた文字・アイコン部分） |
| クリア編成_btn | 幻霊先鋒ステージ.png | 左下の「クリア編成」ボタン |
| オート挑戦_btn | 幻霊先鋒ステージ.png | 下部中央の「オート挑戦」ボタン |
| 一括適用_btn | クリア編成.png | 下部の「一括適用」ボタン |
| オート挑戦中 | オート挑戦中.png | 下部「オート挑戦中...」テキスト |
| オート戦闘終了 | オート戦闘終了.png | タイトル「オート戦闘終了」文字 |
| タップで閉じる | オート戦闘終了.png | 下部「タップで閉じる」テキスト |
| 戦闘敗北 | 戦闘敗北.png | タイトル「戦闘敗北」文字 |
| もう一度_btn | 戦闘敗北.png | 右下の「もう一度」ボタン |

### 5. 自動操作の実行
AFK Journey を起動し、「ステージ選択」画面（幻霊挑戦・挑戦ボタンが見える状態）にしてから、**管理者権限のターミナル**で実行：

```
# 幻霊挑戦ボタンをクリック（デフォルト）
python afk_journey_auto.py

# 挑戦ボタンをクリック
python afk_journey_auto.py --step1 挑戦

# 幻霊挑戦 / 挑戦 をランダムに選択してクリック
python afk_journey_auto.py --step1 random
```

> ⚠️ **管理者権限が必須です。** 通常権限で実行するとクリックがゲームに届きません。
> Git Bash / PowerShell を右クリック →「管理者として実行」で開いてください。

---

## 停止方法
- `Ctrl + C` を押す
- マウスを**画面の左上角**に移動する（フェイルセーフ）

---

## 注意事項

- **ゲーム画面の解像度を変えないこと**（テンプレート画像と一致しなくなる）
- **ウィンドウモードで動作確認済み**（全画面モードは未検証）
- PCのスリープ・スクリーンセーバーはOFFにしてください
- ゲームのアップデートでUIが変わった場合はテンプレートを作り直してください

---

## トラブルシューティング

| 症状 | 対処 |
|---|---|
| 「テンプレート画像が見つかりません」 | make_templates.py を再実行してください |
| ボタンをクリックしてくれない | afk_journey_auto.py の MATCH_THRESHOLD を 0.75 など低くしてみる |
| 誤検知が多い | MATCH_THRESHOLD を 0.85 など高くしてみる |
| 戦闘終了を検知しない | MAX_BATTLE_WAIT を増やす（デフォルト3600秒） |
