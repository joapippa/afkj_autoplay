...
> ⚠ **この集計値はまだ検証中です。** 実測でずれ方が2通り出ています。
>
> | いつ | ゲームの集計画面 | こちらの計上 |
> |---|---|---|
> | 旧版で確認したとき | 合計2 / 合計4ステージクリア | **0回**（少なすぎる） |
> | 2026-08-11 朝 | 合計4ステージクリア（54 → 58） | **9回**（約2.2倍） |
> | 2026-08-11 昼 | 進捗 77 → 165（＝88ステージ） | **154回**（約1.7倍） |
>
> 常に2倍前後に膨らむことから「1回の進行を2回数えている」と見て、
> **同じ新しい看板が2コマ続いたときだけ数える**ように直しました（下記）。
> 効果は次の実走で、実行終了時に出る
> 「看板の観測: 写っていた N回 / 写っていなかった M回 / 裏が取れず見送った変化 K回」
> と集計画面の進捗を突き合わせて確認します。
>
...
**変化は2コマ続けて確認できたときだけ数えます。**（2026-08-11 に修正）

1コマの変化で即座に数えると、看板が描き変わる途中のコマで1回、描き切った
コマでもう1回と、1回の進行を2回数えてしまいます。実測の「常に2倍前後」は
これで説明がつきます。そこで:

| 見えたもの | どうするか |
|---|---|
| 基準と違う看板 | まだ数えない。次のコマまで保留する |
| 保留と同じ看板が続いた | ここで1回数え、基準を入れ替える |
| 基準の看板に戻った | ちらつきだったとみなして見送る（回数を記録） |
| さらに別の看板 | まだ移り変わっている途中。今のコマを保留にして待つ |

見送った回数は実行終了時のサマリに出るので、数えすぎ／数えなさすぎの
切り分けに使えます。
